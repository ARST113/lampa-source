(function () {
  ...
  (function () {
    ...
    function cached(key, size, def) {
      var out;
      try {
        out = Lampa.Storage.cache(key, size, def);
      } catch (e) {
        out = null;
      }
      if (!out || _typeof(out) !== 'object') {
        out = def;
        try {
          Lampa.Storage.set(key, def);
        } catch (e) {}
      }
      return out;
    }
    ...
    function timelineRoad(hash, fallbackPercent) {
      var road = null;
      if (hash) {
        try {
          if (Lampa.Timeline && typeof Lampa.Timeline.view === 'function') {
            road = Lampa.Timeline.view(hash);
          }
        } catch (e) {
          road = null;
        }
      ...
      if (road) {
        return {
          hash: road.hash || hash,
          percent: normalizePercent(road.percent),
          time: Math.max(0, parseFloat(road.time) || 0),
          duration: Math.max(0, parseFloat(road.duration) || 0),
          updated: Math.max(0, parseFloat(road.updated) || 0)
        };
      }
      return {
        hash: hash || '',
        percent: normalizePercent(fallbackPercent),
        time: 0,
        duration: 0,
        updated: 0
      };
    }
    function movieIdentity(card) {
      if (!card) return '';
      var kind = String(card.media_type || card.type || (card.name || card.original_name ? 'tv' : 'movie'));
      var id = card.id || card.tmdb_id || card.kinopoisk_id || card.imdb_id || card.original_title || card.original_name || card.title || card.name || '';
      return id ? kind + ':' + String(id) : '';
    }
    function stableMovieTitle(card) {
      if (!card) return '';
      var key = movieIdentity(card);
      var title = String(card.title || card.name || card.original_title || card.original_name || '').trim();
      if (!key) return title;
      if (!TITLE_MEMORY[key] && title) TITLE_MEMORY[key] = title;
      return TITLE_MEMORY[key] || title;
    }
    function stableItemTitle(item) {
      var title = String(item && item.title || '').trim();
      if (!title) return stableMovieTitle(movie);
      var media = movieIdentity(movie);
      var seat = item.hash || (item.folder ? 'folder:' : 'item:') + String(item.num || item.index || 0);
      var key = media && seat ? media + '|' + seat : '';
      if (!key) return title;
    ...
    function stableClarificationValue(card, value, stored) {
      ...
      return stored ? stableExtraValue(key, stored, true) : stableExtraValue(key, value, false);
    }
    function clarificationStorageValue(card) {
      if (!card) return '';
      var all = get('clarification_search', {});
      if (!all || _typeof(all) !== 'object') return '';
    ...
    function heroEnabled() {
      return get('nova_skin_hero', true) !== false;
    }
    function artEnabled() {
      return get('nova_skin_hero_art', true) !== false;
    }
    function viewMode() {
      return get('nova_skin_view', 'list');
    }
    function preferredQuality() {
      return get('nova_skin_quality', 'auto');
    }
    function focusRing() {
      return get('nova_focus_style', 'ring') !== 'fill';
    }
    function fullScreen() {
      return get('nova_skin_fullscreen', true) === true;
    }
    function applyFullScreen() {
      try {
        var body = $('body');
        if (fullScreen()) body.addClass('nova-skin-full');else body.removeClass('nova-skin-full');
      } catch (e) {}
    }
    function edgeFade() {
      return get('nova_skin_fade', false) === true;
    }
    function applyEdgeFade() {
      try {
        var body = $('body');
        if (edgeFade()) body.addClass('nova-skin-fade');else body.removeClass('nova-skin-fade');
      } catch (e) {}
    }
    function applyFocusStyle() {
      try {
        var body = $('body');
        if (focusRing()) body.addClass('nova-focus-ring');else body.removeClass('nova-focus-ring');
      } catch (e) {}
    }
    var OWN = {
      nova_skin_watch: {
      ...
      nova_skin_from_start: {
        ru: 'Смотреть с начала',
        uk: 'Дивитися з початку',
        en: 'Watch from start'
      },
      nova_skin_next_episode: {
        ru: 'Следующая',
        uk: 'Наступна',
        en: 'Next'
      },
      ...
      nova_skin_more_sources: {
        ru: 'Ещё {count}',
        uk: 'Ще {count}',
        en: '{count} more'
      },
      nova_skin_season_progress: {
        ru: 'Просмотрено {seen} из {total}',
        uk: 'Переглянуто {seen} з {total}',
        en: 'Watched {seen} of {total}'
      },
      nova_skin_season_left: {
        ru: 'осталось {left}',
        uk: 'залишилось {left}',
        en: '{left} left'
      },
      nova_skin_season_planned: {
      ...
      nova_skin_loading_title: {
        ru: 'Ищем, где посмотреть',
        uk: 'Шукаємо, де подивитися',
        en: 'Looking for a source'
      },
      nova_skin_loading_start: {
      ...
      nova_skin_voice_other: {
        ru: 'Другое',
        uk: 'Інше',
        en: 'Other'
      },
      nova_skin_try_source: {
        ru: 'Попробовать {name}',
        uk: 'Спробувати {name}',
        en: 'Try {name}'
      },
      nova_skin_all_sources: {
        ru: 'Все источники',
        uk: 'Всі джерела',
        en: 'All sources'
      },
      ...
      nova_skin_auto_next: {
        ...
        uk: 'Через {sec} с перейдемо на «{name}»',
        en: 'Switching to "{name}" in {sec}s'
      },
      nova_skin_dead_all: {
        ru: 'Ни один источник ничего не нашёл',
        uk: 'Жодне джерело нічого не знайшло',
        en: 'No source found anything'
      },
      nova_skin_set_enable: {
        ru: 'Включить Nova Skin',
        uk: 'Увімкнути Nova Skin',
        en: 'Enable Nova Skin'
      },
      nova_skin_set_enable_descr: {
        ru: 'Единый интерфейс для всех онлайн-плагинов',
        uk: 'Єдиний інтерфейс для всіх онлайн-плагінів',
        en: 'One interface for every online plugin'
      },
      nova_skin_set_enable_descr_alt: {
        ru: 'Единый интерфейс для всех онлайн-плагинов, кроме тех, где он уже встроен',
        uk: 'Єдиний інтерфейс для всіх онлайн-плагінів, крім тих, де він уже вбудований',
        en: 'One interface for every online plugin except those that already ship with it'
      },
      nova_skin_set_hero: {
        ru: 'Шапка с кнопкой',
        uk: 'Шапка з кнопкою',
        en: 'Header with button'
      },
      nova_skin_set_hero_descr: {
        ru: 'Кадр, прогресс и кнопка продолжения сверху',
        uk: 'Кадр, прогрес і кнопка продовження вгорі',
        en: 'Backdrop, progress and continue button on top'
      },
      nova_skin_set_hero_art: {
        ru: 'Кадр в шапке',
        uk: 'Кадр у шапці',
        en: 'Backdrop in header'
      },
      nova_skin_set_hero_art_descr: {
        ru: 'Выключите для компактной шапки без картинки',
        uk: 'Вимкніть для компактної шапки без картинки',
        en: 'Turn off for a compact header without artwork'
      },
      nova_skin_set_logo: {
      ...
      nova_skin_set_art_size: {
        ru: 'Качество кадра',
        uk: 'Якість кадру',
        en: 'Backdrop quality'
      },
      nova_skin_set_art_size_descr: {
      ...
      nova_skin_set_probe: {
        ru: 'Проверять источники в фоне',
        uk: 'Перевіряти джерела у фоні',
        en: 'Check sources in background'
      },
      nova_skin_set_probe_descr: {
        ru: 'Отмечать рабочие точкой и показывать их качество',
        uk: 'Позначати робочі точкою та показувати їхню якість',
        en: 'Mark working ones with a dot and show their quality'
      },
      nova_skin_set_probe_ext: {
        ru: 'Источники проверяет сам онлайн-плагин, повторный обход не нужен',
        uk: 'Джерела перевіряє сам онлайн-плагін, повторний обхід не потрібен',
        en: 'The online plugin checks sources itself, no second pass needed'
      },
      nova_skin_set_switch: {
        ru: 'Автопереход по источникам',
        uk: 'Автоперехід по джерелах',
        en: 'Auto switch source'
      },
      nova_skin_set_switch_descr: {
        ru: 'Если ничего не найдено, пробовать следующий рабочий источник',
        uk: 'Якщо нічого не знайдено, пробувати наступне робоче джерело',
        en: 'Try the next working source when nothing is found'
      },
      nova_skin_set_view: {
        ru: 'Вид списка',
        uk: 'Вигляд списку',
      ...
      nova_skin_set_view_descr: {
        ru: 'Список или плитка (4 в ряд)',
        uk: 'Список або плитка (4 в ряд)',
        en: 'List or grid (4 per row)'
      },
      nova_skin_set_quality: {
        ru: 'Качество по умолчанию',
        uk: 'Якість за замовчуванням',
        en: 'Default quality'
      },
      nova_skin_set_quality_descr: {
        ru: 'Предпочтительное качество воспроизведения',
        uk: 'Бажана якість відтворення',
        en: 'Preferred playback quality'
      },
      nova_skin_set_quality_auto: {
        ru: 'Авто',
        uk: 'Авто',
        en: 'Auto'
      },
      ...
      nova_skin_set_focus_ring: {
        ru: 'Ободок',
        uk: 'Обідок',
        en: 'Outline'
      },
      nova_skin_set_focus_fill: {
        ru: 'Белая заливка',
        uk: 'Біла заливка',
        en: 'White fill'
      }
    ...
    function esc(value) {
      return (value === undefined || value === null ? '' : String(value)).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
    }
    ...
    function image(path, size) {
      ...
      if (path.indexOf('//') === 0) {
        var proto = 'https:';
        try {
          proto = window.location.protocol === 'http:' ? 'http:' : 'https:';
        } catch (e) {}
        return proto + path;
      }
    ...
    function qualityBox() {
      ...
      for (key in all) {
        var entry = all[key];
        if (!entry || _typeof(entry) !== 'object' || !entry.list || now - (entry.t || 0) > QUALITY_TTL) delete all[key];
      }
      return all;
    }
    ...
    function rememberQuality(name, label) {
      ...
      var all = qualityBox();
      var mine = all[id];
      if (!mine || _typeof(mine) !== 'object' || !mine.list) mine = {
        t: 0,
        list: {}
      };
    ...
    function probeAlly(id) {
      ...
      PROBE_ALLY_KEYS.forEach(function (key) {
        var all;
        try {
          all = Lampa.Storage.cache(key, 2000, {});
        } catch (e) {
          all = null;
        }
        if (!all || _typeof(all) !== 'object') return;
        var mine = all[id];
        if (!mine || _typeof(mine) !== 'object' || !mine.list) return;
    ...
    function probeCache(id) {
      ...
      var mine = all[id];
      if (!mine || _typeof(mine) !== 'object' || !mine.list) {
        mine = {
          time: Date.now(),
          list: {}
        };
    ...
    function dropShow() {
      ...
      try {
        ...
        if (fits) {
          ...
          if (last && $.contains(node, last) && scrollSeen(last) && last.getBoundingClientRect().top > node.getBoundingClientRect().top + 4) return;
        }
        var scroll = activeScroll(node);
        if (scroll) {
          try {
    ...
    function artFields() {
      if (!movie) return [];
      return [movie.backdrop_path, movie.poster_path, movie.img, movie.background_image, movie.backdrop, movie.poster, movie.still_path, movie.cover, movie.background];
    }
    function pickArt(size) {
      ...
      try {
        return Lampa.Utils.cardImgBackground(movie) || '';
      } catch (e) {}
      return '';
    }
    function heroArt() {
    ...
    function runtimeText(seconds) {
      try {
        return Lampa.Utils.secondsToTime(seconds, true);
      } catch (e) {
        return '';
      }
    }
    ...
    function handWatch() {
      ...
      names.forEach(function (name) {
        try {
          window.addEventListener(name, handMark, {
            passive: true,
            capture: true
          });
        } catch (e) {
          try {
            window.addEventListener(name, handMark, true);
          } catch (err) {}
        }
      });
    }
    ...
    function lockWatch() {
      ...
      lock_timer = setInterval(function () {
        ...
        if (here.length && here[0] !== wanted[0] && here.closest('.nova-drop').length > 0) return lockStopWatch(true);
        if (here.length && !heroKey(here.attr('data-nova-focus') || '') && here.closest('.nova-toolbar,.nova-drop').length === 0) {
    ...
    function bind(element, enter, _long) {
      element.on('hover:enter', function () {
        ...
        if (lockActive() && key !== ui_lock) {
          ...
          if (!focusing && !pressNow() && ui_open) {
            try {
              stolen = $(e.target).closest('.nova-toolbar').length > 0;
            } catch (e2) {
              stolen = false;
            }
          }
    ...
    function rebind() {
      ...
      ui.rows.find('.nova-toolbar [data-nova-focus]').each(function () {
        ...
        bind(box, action ? action.enter : function () {
          uiToggle(key);
    ...
    function switchWatch() {
      ...
      try {
        ...
        switch_observer.observe(target, {
          childList: true,
          subtree: true
        });
      } catch (e) {
    ...
    function switchMark(on) {
      try {
        ...
        if (on && !has) body.className = (body.className ? body.className + ' ' : '') + name;else if (!on && has) body.className = list.split(' ' + name + ' ').join(' ').replace(/^\s+|\s+$/g, '');
      } catch (e) {}
    }
    var inplace = false;
    ...
    function hostTimerStop(_native) {
      try {
        _native.node.find('.online-empty__button.cancel').trigger('hover:enter');
      } catch (e) {}
      try {
        if (Lampa.Timer && typeof Lampa.Timer.remove === 'function') {
          _native.node.find('.timeout').text('');
        }
      } catch (e) {}
    }
    function probeOn() {
    ...
    function probeMode() {
      var hook = probeHook();
      if (hook) return hook;
      var stamp = [movie ? movie.id : '', (groups.sort || []).length, currentSourceKey(), probe_saves].join('|');
    ...
    function probeMark(key, state) {
      ...
      if (value) {
        ...
        if (badge.length) badge.text(value);else chip.prepend($('<span class="nova-chip__badge"></span>').text(value));
      }
      if (state === 'empty') chip.addClass('nova-chip--empty');else if (state === 'ok') chip.append('<span class="nova-chip__dot"></span>');
    }
    function probeAnswer(key, answer) {
      ...
      var found = (body.match(/videos__item/g) || []).length || (body.match(/"method"\s*:\s*"(play|call|link)"/g) || []).length;
      if (found) {
        var best = '';
    ...
    function probeRun() {
      ...
      var _step = function step() {
        ...
        probe_nets.push(net);
        try {
          net.timeout(PROBE_TIMEOUT);
        } catch (e) {}
        var done = function done(answer) {
      ...
      for (var worker = 0; worker < PROBE_PARALLEL; worker++) _step();
    }
    ...
    function reloadInPlace(comp) {
      ...
      chain.then(function () {
        try {
          if (typeof comp.search === 'function') comp.search();else comp.find();
        } catch (e) {}
      })['catch'](function (error) {
        try {
          if (typeof comp.noConnectToServer === 'function') comp.noConnectToServer(error);else if (typeof comp.empty === 'function') comp.empty();
        } catch (e) {}
      });
      return true;
    }
    ...
    function listFree() {
      gentle_until = 0;
      try {
        if (ui.list) ui.list.css('min-height', '');
      } catch (e) {}
    }
    ...
    function fallbackQuality(origin) {
      var found = '';
      try {
        found = shortQuality(origin.find('.online-prestige__info').text() + ' ' + origin.find('.online-prestige__title').text());
      } catch (e) {
        found = '';
      }
    ...
    function readCard(node, index) {
      ...
      var num = digits(origin.find('.online-prestige__episode-number').text()) || index + 1;
      var raw = (line.children('div').first().attr('style') || '').match(/([\d.]+)%/);
      var fallbackPercent = raw ? parseFloat(raw[1]) || 0 : 0;
      if (!hash && movie && Lampa.Utils && typeof Lampa.Utils.hash === 'function') {
        var timelineTitle = movie.original_title || movie.original_name || movie.title || movie.name || '';
        var series = !!(movie.name || movie.original_name || movie.number_of_seasons);
        var season = seasonNumber() || 1;
        if (timelineTitle) {
          hash = Lampa.Utils.hash(series ? [season, num, timelineTitle].join('') : timelineTitle);
        }
      }
      var timeline = timelineRoad(hash, fallbackPercent);
      var meta = [];
      ...
      return {
        ...
        meta: meta,
        time: soon ? origin.find('.online-prestige__quality').text().trim() : origin.find('.online-prestige__time').text().trim() || (origin.hasClass('online-prestige--folder') ? '' : fallbackTime()),
        quality: soon ? '' : origin.find('.online-prestige__quality').text().trim() || (origin.hasClass('online-prestige--folder') ? '' : fallbackQuality(origin)),
    ...
    function dedupeItems(list) {
      ...
      (list || []).forEach(function (item) {
        if (!item) return;
        var key = item.hash || (item.folder ? 'folder:' : 'item:') + String(item.num || item.index || 0);
    ...
    function percentOf(item) {
      if (!item) return 0;
      return normalizePercent(item.percent);
    }
    ...
    function seasonText(season) {
      var line = text('nova_season_progress', 'nova_skin_season_progress').replace('{seen}', season.seen).replace('{total}', season.total);
      if (season.seen < season.total) {
        line += " \xB7 " + text('nova_season_left', 'nova_skin_season_left').replace('{left}', season.total - season.seen);
      }
      if (season.planned > season.total) {
        line += " \xB7 " + text('nova_season_planned', 'nova_skin_season_planned').replace('{planned}', season.planned);
      }
      return line;
    }
    ...
    function pickResume(full) {
      ...
      for (i = 0; i < list.length; i++) {
        if (!isStarted(list[i])) continue;
        if (!started || resumeUpdated(list[i]) > resumeUpdated(started) || resumeUpdated(list[i]) === resumeUpdated(started) && list[i].index > started.index) {
          started = list[i];
        }
      }
      if (started) return started;
      if (!serial) return list[0];
      for (i = 0; i < list.length; i++) {
        if (!isSeen(list[i])) return list[i];
      }
      return list[list.length - 1];
    }
    function resumeUpdated(item) {
      var value = parseFloat(item && item.updated);
    ...
    function play(item) {
      if (!item) return;
      try {
        item.origin.trigger('hover:enter');
      } catch (e) {}
    }
    function longPress(item) {
      if (!item) return;
      try {
        item.origin.trigger('hover:long');
      } catch (e) {}
    }
    ...
    function paintThumb(thumb, art, fallback) {
      ...
      if (!art || !img) return;
      if (fallback) thumb.addClass('nova-card__thumb--fallback');else thumb.removeClass('nova-card__thumb--fallback');
      img.onload = function () {
    ...
    function watchNumber(item, slot) {
      ...
      function take() {
        var value = 0;
        try {
          value = digits(item.origin.find('.online-prestige__episode-number').text());
        } catch (e) {
          value = 0;
        }
    ...
    function metaFlat(value) {
      return String(value == null ? '' : value).replace(/\s+/g, ' ').replace(/[\s.,;:\u00b7\u25cf|-]+$/, '').trim().toLowerCase();
    }
    ...
    function buildCard(item, compact, grid) {
      var card = $('<div class="nova-card selector">' + '<div class="nova-card__thumb"><img alt=""><div class="nova-card__num"><span></span></div><div class="nova-card__line"></div></div>' + '<div class="nova-card__body"><div class="nova-card__title"></div><div class="nova-card__meta"></div></div>' + '<div class="nova-card__side"><div class="nova-card__quality"></div><div class="nova-card__time"></div></div>' + '</div>');
      ...
      if (item.percent > 0 && item.percent < SEEN_PERCENT && item.time) {
        var duration = item.timeline && item.timeline.duration || digitsTime(item.time);
        var left = Math.round((100 - item.percent) / 100 * duration);
      ...
      if (badge) card.find('.nova-card__quality').addClass('nova-badge').text(badge);else card.find('.nova-card__quality').remove();
      card.find('.nova-card__time').text(item.time || '');
      var line = card.find('.nova-card__line');
      if (item.hash && Lampa.Timeline && typeof Lampa.Timeline.render === 'function') {
        line.append(Lampa.Timeline.render(item.timeline));
        if (!grid) line.addClass('nova-card__line--body').appendTo(body);
      } else if (item.line.length) {
      ...
      if (item.folder) {
        ...
        if (item.viewed || isSeen(item)) {
          if (grid) thumb.append('<div class="nova-card__viewed">' + ICON.eye + '</div>');else card.find('.nova-card__side').append('<div class="nova-card__eye">' + ICON.eye + '</div>');
        }
      }
      if (item.soon) {
    ...
    function playButton() {
      if (!ui.play || !ui.play.length) {
        ui.play = $('<div class="nova-btn nova-btn--main selector" data-nova-focus="hero">' + ICON.play + '<span class="nova-btn__label"></span></div>');
        bindPlay(ui.play);
      }
      return ui.play;
    }
    ...
    function episodeSuffix(item) {
      ...
      return " \xB7 " + text('torrent_serial_episode', 'nova_skin_episode') + ' ' + item.num;
    }
    function playMenu() {
      ...
      var active;
      try {
        active = Lampa.Controller.enabled().name;
      } catch (e) {
        active = 'content';
      }
      ...
      if (items.length > JUMP_FROM) {
        menu.push({
          title: serial ? text('nova_jump_pick', 'nova_skin_jump_pick') : text('nova_files_pick', 'nova_skin_files_pick'),
          jump: true
        });
      }
      try {
        Lampa.Select.show({
          ...
          onBack: function onBack() {
            try {
              Lampa.Controller.toggle(active);
            } catch (e) {}
          },
    ...
    function logoTone(src, path, done) {
      ...
      var _step2 = function step() {
        ...
        probe.onload = function () {
          var tone = '';
          try {
            tone = logoMeasure(probe);
          } catch (e) {
            tone = '';
          }
          if (tone !== 'invert' && tone !== 'glow' && tone !== 'ok') return _step2();
    ...
    function logoEntry(key) {
      ...
      if (mine && _typeof(mine) === 'object' && typeof mine.p === 'string') return mine;
      var old = cached('nova_skin_logo_cache', 500, {});
      var was = old && old[key];
      return typeof was === 'string' && was ? {
        p: was,
        t: 0
      } : null;
    }
    ...
    function logoRemember(key, path, answered) {
      if (path) LOGO_MEM[key] = path;else if (answered) LOGO_MEM[key] = '';else delete LOGO_MEM[key];
    ...
    function logoUrls(card) {
      ...
      kinds.forEach(function (which) {
        [langs, ''].forEach(function (filter) {
          var url = '';
          try {
            url = Lampa.TMDB.api(which + '/' + id + '/images?api_key=' + Lampa.TMDB.key() + (filter ? '&include_image_language=' + filter : ''));
          } catch (e) {
            url = '';
          }
    ...
    function logoFetch(target, done) {
      ...
      var at = 0;
      var answered = false;
      var finish = function finish(path) {
      ...
      var _step3 = function step() {
        ...
        net.silent(url, function (answer) {
          var list = answer && _typeof(answer) === 'object' ? answer.logos : null;
    ...
    function tmdbKind(target) {
      ...
      if (card.number_of_seasons || card.number_of_episodes || card.seasons || card.first_air_date || card.last_air_date || card.name || card.original_name) return 'tv';
    ...
    function seasonPlanned(season, done) {
      ...
      var api = null;
      try {
        api = Lampa.Api.sources.tmdb;
      } catch (e) {
        api = null;
      }
      if (!api || typeof api.get !== 'function') return done(0);
      var keep = function keep(count) {
    ...
    function logoDraw(box, path, name) {
      ...
      logoTone(src, path, function (tone) {
        ...
        if (tone === 'invert') picture.addClass('nova-logo--invert');else if (tone === 'glow') picture.addClass('nova-logo--glow');else if (tone === 'blind') picture.addClass('nova-logo--edge');
      });
    }
    function heroLogo() {
      var slot = logoSlot();
      if (!slot) return;
      var name = stableMovieTitle(movie);
    ...
    function heroStatic(withArt) {
      ...
      ui.hero.find('.nova-hero__hint').text(hint.join(" \xB7 "));
    ...
    function buildHero() {
      ...
      if (!ui.hero) {
        ui.hero_kind = kind;
        ui.hero = $('<div class="nova-hero">' + (withArt ? '<div class="nova-hero__bg"><img alt=""></div><div class="nova-hero__shade"></div>' : '') + '<div class="nova-hero__body">' + (withArt ? '<div class="nova-hero__title"></div><div class="nova-hero__meta"></div><div class="nova-hero__descr"></div>' : '') + '<div class="nova-hero__actions">' + (withArt ? '' : '<div class="nova-hero__mark"></div>') + '<div class="nova-hero__hint"></div></div>' + '<div class="nova-hero__season" style="display:none"></div>' + '</div>' + '<div class="nova-hero__progress" style="display:none"></div>' + '</div>');
        if (!withArt) ui.hero.addClass('nova-hero--compact');
        if (withArt) {
          ui.hero.find('.nova-hero__title').text(stableMovieTitle(movie));
      ...
      if (next && started) {
        next_button.find('.nova-btn__label').text(text('nova_next_episode', 'nova_skin_next_episode') + episodeSuffix(next));
      ...
      if (serial && target.numbered) {
        label += " \xB7 S" + (seasonNumber() || 1) + ' E' + target.num;
      }
      ...
      var percent = heroProgress(target);
      if (percent > 0) {
        progress.show().append('<div class="time-line"><div style="width:' + percent + '%"></div></div>');
      } else progress.hide();
      return button;
    }
    function heroProgress(target, season) {
      return target ? percentOf(target) : 0;
    }
    function uiToggle(key) {
      ...
      if (opening) {
        ...
        } else if (key === 'season') ui_focus = 'season:' + selectedIndex(groups.season);else if (key === 'voice') ui_focus = 'voice:' + selectedIndex(groups.voice);
      }
      buildRows();
      if (opening) {
        var entry = dropEntry();
      ...
      if (opening && key === 'source') probeRun();else probeStop();
    ...
    function markSourceBusy(item) {
      ...
      try {
        ...
        if (value) {
          if (badge.length) badge.text(value);else box.prepend($('<span class="nova-chip__badge"></span>').text(value));
        } else badge.remove();
      } catch (e) {}
    }
    ...
    function openExtra(index) {
      ...
      capturing = true;
      try {
        entry.node.trigger('hover:enter');
      } catch (e) {}
      capturing = false;
      if (!captured) return;
      switchDone();
    ...
    function sourceRow() {
      ...
      sort.forEach(function (item) {
        ...
        if (sourceActive(item, key, state, graded)) visible.push(item);else hidden.push(item);
      });
      ...
      if (!ui_all_sources && hidden.length) {
        var more = chip('src:more', text('nova_more_sources', 'nova_skin_more_sources').replace('{count}', hidden.length), {
          plain: true
        });
    ...
    function optionRow(group) {
      ...
      order.forEach(function (entry) {
        var box = chip(group.stype + ':' + entry.index, plain_season ? partTitle(entry.item.title) : entry.item.title, {
    ...
    function buildRows() {
      ...
      chip_actions = {};
      var addChip = function addChip(key, title, value, extra) {
        ...
        chip_actions[key] = {
          enter: extra && extra.action ? extra.action : function () {
            uiToggle(key);
          },
      ...
      if (groups.season && groups.season.items.length > 1) {
        if (serial) {
          addChip('season', groups.season.title || text('torrent_serial_season', 'nova_skin_season'), groups.season.subtitle || '', {});
        } else {
          addChip('season', text('nova_playlist', 'nova_skin_playlist'), partTitle(groups.season.subtitle), {});
        }
      }
      if (groups.voice && groups.voice.items.length > 1) {
        addChip('voice', groups.voice.title || text('torrent_parser_voice', 'nova_skin_voice'), groups.voice.subtitle || '', {});
      }
      if (!nav && items.length > JUMP_FROM) {
        var now = pageAt(pages(items.length), ui_page > 0 ? ui_page : 0);
        addChip('jump', serial ? text('nova_jump', 'nova_skin_jump') : text('nova_files', 'nova_skin_files'), pageTitle(now), {});
      }
      rows.append(toolbar);
      fitToolbar(toolbar);
      if (ui_open === 'source') sourceRow();else if (ui_open === 'season' && groups.season) optionRow(groups.season);else if (ui_open === 'voice' && groups.voice) optionRow(groups.voice);else if (ui_open === 'jump') jumpRow();else if (extra_menu && ui_open === extra_menu.key) extraRow();
    }
    function focusNode(target, gentle) {
      ...
      focusing = true;
      try {
        Lampa.Controller.collectionFocus(node, host);
      } catch (e) {}
      focusing = false;
      return true;
    }
    ...
    function refreshCollection() {
      ...
      try {
        Lampa.Controller.collectionSet(host, false, true);
      } catch (e) {}
    }
    function inSkin() {
    ...
    function keepFocus() {
      ...
      if (!wanted || !wanted.length) {
        if (ui.play && ui.play.length && ui.play.parent().length && shown(ui.play)) wanted = ui.play;else {
    ...
    function loadingPanel() {
      ...
      if (!ui.load) {
        ...
        ui.load = $('<div class="nova-loading">' + '<div class="nova-loading__title"></div>' + '<div class="nova-loading__text"></div>' + '<div class="nova-loading__bar"><div></div></div>' + '</div>');
    ...
    function loadingText() {
      ...
      var line = text('nova_loading_start', 'nova_skin_loading_start') + " \xB7 " + seconds + text('nova_sec', 'nova_skin_sec');
    ...
    function noteStamp(_native2) {
      return [_native2.node.find('.online-empty__title').text().trim(), currentSourceKey(), sourceTitle(), (groups.sort || []).length, movie ? movie.id : 0].join('|');
    }
    function notePanel(_native3) {
      var mark = noteStamp(_native3);
      var alive = ui.root && ui.root.parent().length && ui.list && ui.list.find('.nova-note').length;
      if (alive && note_sig === mark) {
        hostTimerStop(_native3);
      ...
      note.find('.nova-note__title').text(_native3.node.find('.online-empty__title').text().trim());
      note.find('.nova-note__text').text(_native3.node.find('.online-empty__time').text().trim());
      var actions = note.find('.nova-note__actions');
      var addAction = function addAction(label, icon, run, keep_hop) {
      ...
      if (next) {
        addAction(text('nova_try_source', 'nova_skin_try_source').replace('{name}', splitSourceName(next.title).name || next.title), ICON.play, function () {
          chooseSource(next);
        });
      }
      ...
      if (auto) {
        ...
        var render = function render() {
          slot.text(text('nova_auto_next', 'nova_skin_auto_next').replace('{sec}', tic).replace('{name}', name));
        };
        render();
    ...
    function stamp() {
      return [items.length, nav ? 'nav' : 'files', sourceTitle(), groups.season ? groups.season.subtitle : '', groups.voice ? groups.voice.subtitle : '', extrasStamp(), viewMode(), ui_open, ui_page, ui_all_sources ? 1 : 0, items.length ? stableItemTitle(items[0]) : '', items.filter(function (i) {
        return isSeen(i);
      }).length].join('|');
    }
    function draw() {
      ...
      extras = readExtras();
      var _native4 = nativeState();
      if (_native4) {
        busy = true;
        knownRemember();
        root.addClass('nova-skin-scope nova-skin-chips');
        _native4.node.addClass('nova-hidden');
        items = [];
        signature = '';
        if (_native4.kind === 'loading') {
          if (pendingLive() && ui.list && ui.list.find('.nova-skeleton').length) {
            busy = false;
            return;
          }
          loadingPanel();
        } else notePanel(_native4);
        busy = false;
        return;
      }
      var list = collect();
      ...
      if (grid) ui.list.addClass('nova__list--grid');else ui.list.removeClass('nova__list--grid');
      ...
      observer = new MutationObserver(function (records) {
        for (var i = 0; i < records.length; i++) {
          ...
          if (node && node.nodeType === 1 && $(node).closest('.nova-skin-root').length) {
            if ($(node).closest('.nova-chip--clarification').length || $(node).find('.nova-chip--clarification').length) {
    ...
    function refreshMarks() {
      ...
      items.forEach(function (item) {
        ...
        var timeline = timelineRoad(item.hash, item.percent);
        item.timeline = timeline;
        item.percent = timeline.percent;
        item.updated = timeline.updated;
        if (item.origin && item.origin.find('.online-prestige__viewed').length) item.viewed = true;
        var card = item.card;
        if (!card || !card.length) return;
        var box = card.find('.nova-card__line');
        if (box.length && item.hash && Lampa.Timeline && typeof Lampa.Timeline.render === 'function') {
          box.empty().append(Lampa.Timeline.render(item.timeline));
        } else if (box.length && item.line && item.line.length) {
    ...
    function afterPlayerStop() {
      after_player.forEach(function (id) {
        clearTimeout(id);
      });
      after_player = [];
      ...
      try {
        ...
        Lampa.Player.listener.follow('start', function (data) {
          try {
            ...
            if (!want || !data || !data.quality || _typeof(data.quality) !== 'object') return;
      ...
      try {
        ...
        Lampa.SettingsApi.addParam({
          ...
          field: {
            ...
            description: siblingInstalled() ? label('nova_skin_set_enable_descr_alt') : label('nova_skin_set_enable_descr')
          },
    ...
    var LOGO_HOLD = '.nova-skin-root .nova-loading,.nova-skin-root .nova-note{width:100%;-webkit-box-flex:1;-webkit-flex:1 1 100%;-ms-flex:1 1 100%;flex:1 1 100%;min-width:0}' + '.nova-skin-root .nova-skeleton{width:100%;-webkit-box-flex:1;-webkit-flex:1 1 100%;-ms-flex:1 1 100%;flex:1 1 100%;min-width:0}' + '.nova-skin-root .nova-hero__title{min-height:2.3em}' + '.nova-skin-root .nova-hero__title--logo{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-box-align:end;-webkit-align-items:flex-end;-ms-flex-align:end;align-items:flex-end;min-height:2.3em}' + '.nova-skin-root .nova-hero__mark.nova-hero__title--logo{min-height:0}' + '@media screen and (max-width:580px){.nova-skin-root .nova-hero__title{min-height:2.1em}.nova-skin-root .nova-hero__title--logo{min-height:2.1em}}' + '.nova-skin-root .nova-skeleton--grid{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;margin:0 -.45em}' + '.nova-skin-root .nova-skeleton--grid .nova-skeleton__row{display:block;width:25%;margin:0 0 1em 0;padding:0 .45em;background:none;-webkit-border-radius:0;border-radius:0}' + '.nova-skin-root .nova-skeleton--grid .nova-skeleton__thumb{width:100%;height:0;padding-top:56%;-webkit-border-radius:.5em;border-radius:.5em}' + '.nova-skin-root .nova-skeleton--grid .nova-skeleton__body{padding:.5em .1em 0 .1em}' + '.nova-skin-root .nova-skeleton--grid .nova-skeleton__line{height:.8em;margin-bottom:.4em}' + '@media screen and (max-width:860px){.nova-skin-root .nova-skeleton--grid .nova-skeleton__row{width:33.3333%}}' + '@media screen and (max-width:580px){.nova-skin-root .nova-skeleton--grid .nova-skeleton__row{width:50%}}';
    var BADGE_CSS = ['.nova-skin-root .nova__list--grid .nova-card__num{padding:.45em 0 0 .5em;font-size:1em}', '.nova-skin-root .nova__list--grid .nova-card__side{top:.45em;right:.95em;text-align:right}', '.nova-skin-root .nova__list--grid .nova-card__num>span,.nova-skin-root .nova__list--grid .nova-card__quality{display:inline-block;font-size:1em;font-weight:600;line-height:1.3;letter-spacing:.02em;padding:.1em .42em;-webkit-border-radius:.35em;border-radius:.35em;background:rgba(10,11,17,.62);-webkit-box-shadow:0 .12em .45em rgba(0,0,0,.4);box-shadow:0 .12em .45em rgba(0,0,0,.4);text-shadow:none;color:#fff}', '.nova-skin-root .nova__list--grid .nova-card__thumb:not(.nova-card__thumb--loaded) .nova-card__num>span,.nova-skin-root .nova__list--grid .nova-card__thumb--fallback .nova-card__num>span{background:rgba(10,11,17,.62);padding:.1em .42em;-webkit-box-shadow:0 .12em .45em rgba(0,0,0,.4);box-shadow:0 .12em .45em rgba(0,0,0,.4)}'].join('');
    ...
    var EXTRA_CSS = ".nova-skin-root .nova-btn>svg{width:1.15em!important;height:1.15em!important;max-width:1.15em;max-height:1.15em;-webkit-box-flex:0;-webkit-flex:0 0 auto;-ms-flex:0 0 auto;flex:0 0 auto}.nova-skin-root .nova-chip>svg{width:1em!important;height:1em!important;max-width:1em;max-height:1em;-webkit-box-flex:0;-webkit-flex:0 0 auto;-ms-flex:0 0 auto;flex:0 0 auto}.nova-skin-root .nova-card__eye>svg{width:1.2em!important;height:1.2em!important}.nova-skin-root .nova-card__viewed>svg{width:100%!important;height:100%!important}.nova-skin-root .nova-btn__label{min-width:0;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis}.nova-skin-chips .explorer__files-head{display:none!important}.nova-skin-scope .nova-hidden{display:none!important}.nova-skin-scope .nova{padding-top:0}.nova-skin-root .nova-card__line>.time-line{display:block!important;height:100%;margin:0;background:none}.nova-skin-root .nova-card__line>.time-line>div{height:100%;background:#fff}.nova-skin-root .nova-card__line--body>.time-line>div{background:rgba(255,255,255,.9)}.nova-skin-root .nova-hero__progress .time-line{display:block!important;height:100%;margin:0;background:none}.nova-skin-root .nova-hero__progress .time-line>div{height:100%;background:#fff}.nova-skin-scope .explorer__files-body .scroll__body>*{display:none!important}.nova-skin-scope .explorer__files-body .scroll__body>.nova-skin-root{display:block!important}.nova-skin-scope .explorer__files-body .broadcast__scan{display:none!important}.nova-skin-scope .explorer__files-head{display:none!important}.nova-skin-switching .activity--active{-webkit-animation:none!important;animation:none!important;-webkit-transition:none!important;transition:none!important;-webkit-transform:none!important;transform:none!important}.nova-skin-switching .activity:not(.activity--active){opacity:0!important}.nova-skin-switching .explorer__left,.nova-skin-switching .explorer__files{-webkit-transition:none!important;transition:none!important;-webkit-animation:none!important;animation:none!important}.nova-skin-root .nova-toolbar .nova-chip{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;max-width:none}.nova-skin-root .nova-toolbar--clip .nova-chip{-webkit-flex-shrink:1;-ms-flex-negative:1;flex-shrink:1;min-width:4.5em}.nova-skin-root .nova-toolbar--clip .nova-chip__label{overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;min-width:0}.nova-skin-root .nova-hero__title,.nova-skin-root .nova-hero__meta,.nova-skin-root .nova-hero__descr,.nova-skin-root .nova-hero__hint,.nova-skin-root .nova-hero__season{text-shadow:0 .06em .5em rgba(0,0,0,.8)}.nova-skin-root .nova-hero__descr{opacity:.8}.nova-skin-root .nova-hero__title--logo{display:block;overflow:visible;-webkit-line-clamp:none;-webkit-box-orient:horizontal;padding:.08em 0 .06em;margin-bottom:.5em;line-height:1}.nova-skin-root .nova-hero__title--logo>img{display:block;max-height:2.1em;max-width:70%;width:auto;height:auto;-o-object-fit:contain;object-fit:contain;image-rendering:-webkit-optimize-contrast;-webkit-filter:drop-shadow(0 .04em .12em rgba(0,0,0,.55));filter:drop-shadow(0 .04em .12em rgba(0,0,0,.55))}.nova-skin-root .nova-hero__title--logo>img.nova-logo--invert{-webkit-filter:invert(1) brightness(1.1) drop-shadow(0 .04em .12em rgba(0,0,0,.5));filter:invert(1) brightness(1.1) drop-shadow(0 .04em .12em rgba(0,0,0,.5))}.nova-skin-root .nova-hero__title--logo>img.nova-logo--glow{-webkit-filter:drop-shadow(0 0 .03em rgba(255,255,255,.5)) drop-shadow(0 0 .07em rgba(255,255,255,.35));filter:drop-shadow(0 0 .03em rgba(255,255,255,.5)) drop-shadow(0 0 .07em rgba(255,255,255,.35))}@media screen and (max-width:580px){.nova-skin-root .nova-hero__title--logo>img{max-height:1.9em;max-width:80%}}.nova-skin-root .nova-chip--checking{opacity:.55}.nova-skin-root .nova-chip--checking .nova-chip__label{opacity:.6}@media screen and (max-width:900px){.nova-skin-root .nova-hero__hint{-webkit-flex:1 0 100%;-ms-flex:1 0 100%;flex:1 0 100%;margin:.3em 0 0 .15em;white-space:normal}}@media screen and (max-width:580px){.nova-skin-root .nova-card__side{display:block!important;text-align:center;padding-right:.2em;max-width:6em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}.nova-skin-root .nova-card__quality{font-size:.66em;padding:.15em .4em}.nova-skin-root .nova-card__time{font-size:.78em;margin-top:.3em;display:block}.nova-skin-root .nova__list--grid .nova-card__side{position:absolute;top:.4em;right:.6em;max-width:none}.nova-skin-root .nova__list--grid .nova-card__time{display:none}}" + ".nova-skin-root .nova-card__go{margin-left:auto;padding-right:.5em;-webkit-align-self:center;-ms-flex-item-align:center;align-self:center}" + ".nova-skin-root .nova__list--grid .nova-card__go{display:none}" + ".nova-skin-root .nova-card--nav .nova-card__thumb--poster{width:4.7em;height:7em;background:rgba(0,0,0,.5)}" + ".nova-skin-root .nova-card--nav .nova-card__thumb--poster img{-o-object-fit:contain;object-fit:contain}" + ".nova-skin-root .nova__list--grid .nova-card--nav .nova-card__thumb--poster{width:100%;height:auto;padding-top:150%}" + "@media screen and (max-width:580px){.nova-skin-root .nova-card--nav .nova-card__thumb--poster{width:3.7em;height:5.5em}}";
    var FOCUS_CSS = ['body.nova-focus-ring .nova-btn.focus,body.nova-focus-ring .nova-chip.focus,body.nova-focus-ring .nova-card.focus,body.nova-focus-ring .nova-group.focus{background:rgba(255,255,255,.16)!important;color:#fff!important;-webkit-box-shadow:inset 0 0 0 .12em #fff!important;box-shadow:inset 0 0 0 .12em #fff!important}', 'body.nova-focus-ring .nova-btn--main{background:rgba(255,255,255,.16);color:#fff;-webkit-box-shadow:inset 0 0 0 .1em rgba(255,255,255,.5);box-shadow:inset 0 0 0 .1em rgba(255,255,255,.5)}', 'body.nova-focus-ring .nova-chip.focus .nova-chip__badge{background:rgba(255,255,255,.2)!important;color:#fff!important}', 'body.nova-focus-ring .nova-card.focus .nova-card__line--body{background:rgba(255,255,255,.2)!important}', 'body.nova-focus-ring .nova-card.focus .nova-card__line--body .time-line>div{background:#fff!important}', 'body.nova-focus-ring .nova-card--slim.focus .nova-card__line{background:rgba(255,255,255,.2)!important}', 'body.nova-focus-ring .nova-card--slim.focus .nova-card__line .time-line>div{background:#fff!important}', 'body.nova-focus-ring .nova__list--grid .nova-card.focus{background:none!important;color:inherit!important;-webkit-box-shadow:none!important;box-shadow:none!important}', 'body.nova-focus-ring .nova__list--grid .nova-card.focus .nova-card__thumb{-webkit-box-shadow:0 0 0 .12em #fff!important;box-shadow:0 0 0 .12em #fff!important}', 'body.nova-focus-ring .nova-chip--active.focus,body.nova-focus-ring .nova-group--open.focus{-webkit-box-shadow:inset 0 0 0 .12em #fff!important;box-shadow:inset 0 0 0 .12em #fff!important}'].join('');
    var CARD_CSS = ['.nova-skin-root .nova-card__viewed{top:auto;bottom:.55em;left:.55em;width:1.15em;height:1.15em;-webkit-border-radius:0;border-radius:0;background:none;opacity:.8;-webkit-box-shadow:none;box-shadow:none}', '.nova-skin-root .nova-card__viewed>svg{display:block;width:100%;height:100%;-webkit-filter:drop-shadow(0 0 .2em rgba(0,0,0,.9));filter:drop-shadow(0 0 .2em rgba(0,0,0,.9))}', '.nova-skin-root .nova-card--soon{opacity:.45}', '.nova-skin-root .nova-card--soon .nova-card__side{max-width:8em;min-width:0;padding-right:.35em;overflow:hidden}', '.nova-skin-root .nova-card--soon .nova-card__time{white-space:normal;overflow-wrap:break-word;word-wrap:break-word;word-break:break-word;line-height:1.25;max-width:100%}', '@media screen and (max-width:580px){.nova-skin-root .nova-card--soon .nova-card__side{max-width:6em;padding-right:.1em}.nova-skin-root .nova-card--soon .nova-card__time{font-size:.72em}}', '@media screen and (max-width:420px){.nova-skin-root .nova-card--soon .nova-card__side{max-width:5em}.nova-skin-root .nova-card--soon .nova-card__time{font-size:.68em}}', '.nova-skin-root .nova-card__eye{display:block;margin-top:.4em;opacity:.5}', '.nova-skin-root .nova-card__eye>svg{display:block;width:1.2em;height:1.2em;margin:0 auto}', '.nova-skin-root .nova-card.focus .nova-card__eye{opacity:.65}', 'body.nova-focus-ring .nova-card.focus .nova-card__eye{opacity:.75!important}'].join('');
    var MARK_CSS = ['.nova-skin-root .nova-hero__mark{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-box-align:center;-webkit-align-items:center;-ms-flex-align:center;align-items:center;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;min-width:0;max-width:40%;margin:0 1.2em .4em 0;font-size:1.2em;font-weight:600;line-height:1.25;overflow:hidden;white-space:nowrap;-o-text-overflow:ellipsis;text-overflow:ellipsis}', '.nova-skin-root .nova-hero__mark:empty{display:none}', '.nova-skin-root .nova-hero__mark.nova-hero__title--logo{display:block;overflow:visible;padding:.08em 0 .06em;margin-bottom:.4em;line-height:1}', '.nova-skin-root .nova-hero__mark>img{max-height:1.75em!important;max-width:100%!important;width:auto;height:auto}', '@media screen and (max-width:900px){.nova-skin-root .nova-hero__mark{max-width:55%;font-size:1.05em}}', '@media screen and (max-width:580px){.nova-skin-root .nova-hero__mark{-webkit-box-ordinal-group:2;-webkit-order:1;-ms-flex-order:1;order:1;max-width:100%;margin-right:0}.nova-skin-root .nova-hero__mark>img{max-height:1.5em!important}}'].join('');
    var FULL_CSS = ['body.nova-skin-full .nova-skin-scope .explorer__left{display:none!important}', 'body.nova-skin-full .nova-skin-scope .explorer__files{width:100%!important;left:0!important}'].join('');
    ...
    var FADE_CSS = ['body.nova-skin-fade .nova-skin-root .nova-hero{background:transparent;-webkit-border-radius:0;border-radius:0}', 'body.nova-skin-fade .nova-skin-root .nova-hero__progress{left:2.2em;right:2.2em;bottom:1.5em;width:auto;-webkit-border-radius:.3em;border-radius:.3em}', 'body.nova-skin-fade .nova-skin-root .nova-hero--compact .nova-hero__progress{left:1.4em;right:1.4em;bottom:.9em}', '@media screen and (max-width:580px){body.nova-skin-fade .nova-skin-root .nova-hero__progress{left:1.3em;right:1.3em;bottom:1em}}', 'body.nova-skin-fade .nova-skin-root .nova-hero__season{margin-bottom:.6em}', 'body.nova-skin-fade .nova-skin-root .nova-hero--compact .nova-hero__season{margin-bottom:1em}', '@media screen and (max-width:580px){body.nova-skin-fade .nova-skin-root .nova-hero__season{margin-bottom:.9em}}', 'body.nova-skin-fade .nova-skin-root .nova-hero__bg{' + FADE_MASK + '}', 'body.nova-skin-fade .nova-skin-root .nova-hero__shade{' + FADE_MASK + '}'].join('');
