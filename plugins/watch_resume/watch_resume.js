...
(function (root, factory) {
  ...
  function sanitizeObject(value, depth) {
    ...
    if (Array.isArray(value)) {
      return value.map(function (item) { return sanitizeObject(item, depth + 1); })
  ...
  function containsForbiddenKey(value, depth) {
    ...
    if (Array.isArray(value)) {
      return value.some(function (item) { return containsForbiddenKey(item, depth + 1); });
    }
  ...
  function extractMagnetInfoHash(input) {
    ...
    if (!match) return '';
    try { return normalizeInfoHash(decodeURIComponent(match[1])); }
    catch (_) { return normalizeInfoHash(match[1]); }
  }
  ...
  function parseTorrServerStream(rawUrl) {
    ...
    if (streamMatch && streamMatch[1]) {
      try { fileName = decodeURIComponent(streamMatch[1]); }
      catch (_) { fileName = streamMatch[1]; }
    }
  ...
  function episodeIdentity(play) {
    ...
    var haystack = [
      play.title, play.name, play.filename, play.file_name, play.path, play.path_human, play.url
    ...
    return { season: asInt(match[1], 0), episode: asInt(match[2], 0) };
  }
  ...
  function subsetCard(card) {
    card = card || {};
    var allowed = [
      'id', 'tmdb_id', 'imdb_id', 'kinopoisk_id', 'kp_id', 'source', 'type',
      'title', 'name', 'original_title', 'original_name', 'release_date', 'first_air_date',
      'original_language', 'poster_path', 'backdrop_path', 'number_of_seasons'
    ];
    var out = {};
  ...
  function applyResumeTimeline(play, record) {
    ...
    var timeline = {};
    Object.keys(old).forEach(function (key) { timeline[key] = old[key]; });
  ...
  function chooseProgress(localProgress, torrTimecodeSeconds) {
    ...
    if (!local) return tsMs > 0 ? { position_ms: tsMs, duration_ms: 0, percent: 0, completed: false } : null;
  ...
  function recordMatchesCard(record, card) {
    ...
    var cardTmdb = card.tmdb_id != null && String(card.tmdb_id) !== ''
      ? String(card.tmdb_id)
      : (cardSource === 'tmdb' ? cardId : '');
    var mediaTmdb = media.tmdb_id != null && String(media.tmdb_id) !== ''
      ? String(media.tmdb_id)
      : (mediaSource === 'tmdb' ? mediaId : '');
  ...
  function findLatestResumeForCard(records, card, profile, minPositionMs, completionPercent) {
    ...
    filtered.sort(function (a, b) { return asNumber(b.updated_at, 0) - asNumber(a.updated_at, 0); });
...
(function () {
  ...
  function notify(text) {
    try {
      ...
      else console.log('[LampacResume]', text);
    } catch (_) {}
  }
  ...
  function loadDb() {
    ...
    if (!db.hash_map || typeof db.hash_map !== 'object') db.hash_map = {};
  ...
  function cardFromActiveActivity() {
    try {
      ...
      var candidates = [
        active.movie,
        active.card,
        active.object && active.object.movie,
        active.object && active.object.card,
        ...
        active.activity && active.activity.object && active.activity.object.card
      ];
      for (var i = 0; i < candidates.length; i++) {
        var card = candidates[i];
        if (card && typeof card === 'object' && (card.id != null || card.tmdb_id || card.imdb_id || card.kinopoisk_id || card.title || card.name)) return card;
      }
    } catch (e) {
  ...
  function normalizeTorrServerBase(raw) {
    ...
    if (/^\//.test(base)) {
      try { return new URL(base, window.location.origin).toString().replace(/\/+$/, ''); }
      catch (_) { return base; }
    }
  ...
  function torrServerBase() {
    ...
    if (normalized && raw && normalized !== raw) {
      try { Lampa.Storage.set('torrserver_url', normalized); } catch (_) {}
  ...
  function createSourceRecipe(play, card) {
    ...
    if (play && (play.isonline || play.voice_name)) {
      ...
      var recipe = Core.createOnlineRecipe({ card: card, play: play, balancer: activeBalancer });
  ...
  function upsertSession(play) {
    ...
    if (!Core.recordHasNoTransientUrl(record)) {
      ...
      record.source = { kind: source && source.kind || 'generic', adapter: source && source.adapter || 'lampa' };
    }
  ...
  function updateProgress(key, progress, timelineHash) {
    ...
    if (lastKnownCard && Core.recordMatchesCard(record, lastKnownCard) && Core.shouldResume(record.progress, MIN_RESUME_MS, COMPLETION_PERCENT)) {
      setTimeout(function () { refreshCurrentCardButton('progress'); }, 0);
    }
    return record;
  }
  ...
  function onPlayerDestroy() {
    ...
    stopPolling();
    currentSession = null;
    setTimeout(function () { refreshCurrentCardButton('player-destroy'); }, 50);
  }
  ...
  function pushTorrServerViewed(record, force) {
    ...
    var success = function () { log('TorrServer viewed saved', hash, fileIndex, seconds); };
    var failure = function (err) { log('TorrServer viewed save failed', err && (err.message || err.status) || err); };
  ...
  function pullTorrServerViewed(record, callback) {
    ...
    function fallback() {
      requestJson(tsEndpoint(record), {
        ...
        file_index: fileIndex,
        timecode: 0
      }, consume, function () { callback && callback(null); });
    }
  ...
  function indexPlaylist(playlist) {
    ...
    playlist.forEach(function (item) {
      ...
      if (torrent) {
        ...
        source.selection.season_index = activeSource.selection && activeSource.selection.season_index != null
          ? Number(activeSource.selection.season_index) : Number(item.season || 0);
        source.selection.voice_index = activeSource.selection && activeSource.selection.voice_index != null
          ? Number(activeSource.selection.voice_index) : 0;
      ...
      var timelineHash = item.timeline && item.timeline.hash
        ? String(item.timeline.hash)
        : (old && old.timeline_hash ? old.timeline_hash : '');
      var record = {
        key: key,
        profile_id: profileId(),
        media: Core.sanitizeObject(media),
  ...
  function installPlayerPatch() {
    ...
    Lampa.Player.play = function (playData) {
      ...
      catch (e) { log('before play error', e && e.stack || e); }
      return originalPlay.apply(this, arguments);
    };

    if (originalPlaylist) {
      Lampa.Player.playlist = function (playlist) {
        try { indexPlaylist(playlist); }
        catch (e) { log('playlist index error', e); }
    ...
    if (balancer) {
      ...
      if (recipeCard.id != null) {
        ...
        if (Number(selection.season || 0) > 0 && Number(selection.episode || 0) > 0) {
          if (!choice.episodes_view || typeof choice.episodes_view !== 'object') choice.episodes_view = {};
  ...
  function absoluteOnlineUrl(origin, url) {
    url = String(url || '');
    if (!url) return '';
    try { return new URL(url, String(origin || '').replace(/\/+$/, '') + '/').toString(); }
    catch (_) { return url; }
  }
  ...
  function onlineRequestParams(url, card) {
    ...
    args.forEach(function (part) { url = addUrlPart(url, part); });
    return url;
  }
  ...
  function onlineGet(url, success, error, jsonMode) {
    var network = new Lampa.Reguest();
    try { network.timeout(12000); } catch (_) {}
  ...
  function chooseOnlineBalancer(events, wanted) {
    ...
    var visible = list.filter(function (item) { return item && item.show !== false; });
    return visible.filter(function (item) { return onlineBalancerName(item) === target; })[0] || visible[0] || null;
  }
  ...
  function runOnlineRch(origin, json, success, error) {
    ...
    function invoke() {
      try {
        ...
        client = window.nwsClient[key] = new NativeWsClient(json.nws, { autoReconnect: true });
        client.on('Connected', function () { registry.Registry(client, success); });
        client.connect();
      } catch (e) {
        if (error) error(e);
      }
  ...
  function chooseVoice(voices, selection) {
    ...
    return voices[idx] || voices.filter(function (voice) { return voice && voice.active; })[0] || voices[0] || null;
  }
  ...
  function choosePlayableItem(data, record) {
    ...
    if (isTv) {
      ...
      if (exact) return { item: exact, items: data };
      if (episode > 0 && data[episode - 1]) return { item: data[episode - 1], items: data };
    } else {
      ...
      if (wantedVoice) {
        ...
        if (voiceMatch) return { item: voiceMatch, items: data };
      }
    }
    return { item: data[0] || null, items: data };
  }
  ...
  function resolveOnlineListing(url, origin, record, depth, success, error) {
    ...
    log('listing step', { depth: depth, url: safeUrlForLog(listingUrl) });
    onlineGet(listingUrl, function (response) {
      log('listing step response', { depth: depth, response: summarizeOnlineResponse(response) });
      ...
      success({ response: response, item: chosen.item, items: chosen.items });
    }, error, true);
  }
  ...
  function setOnlineDefaultQuality(data) {
    if (!data || !data.quality || typeof data.quality !== 'object') return data;
    var wanted = 0;
    try { wanted = parseInt(Lampa.Storage.field('video_quality_default'), 10) || 0; } catch (_) {}
  ...
  function onlineTimelineForItem(file, record, selected) {
    ...
    if (selected && record && record.timeline_hash) {
      ...
      return { hash: String(record.timeline_hash), time: 0, duration: 0, percent: 0 };
    }
    if (file.timeline && typeof file.timeline === 'object') return file.timeline;
    ...
    var raw = season
      ? [season, season > 10 ? ':' : '', episode, originalTitle].join('')
      : originalTitle;
    var hash = raw;
    try {
    ...
    return { hash: String(hash), time: 0, duration: 0, percent: 0 };
  }
  ...
  function resolveOnlineFile(file, origin, success, error, rchRetried) {
    ...
    log('file resolver', { method: file.method, season: file.season, episode: file.episode, url: safeUrlForLog(resolverUrl) });
    onlineGet(resolverUrl, function (json) {
      if (typeof json === 'string' && /^https?:\/\//i.test(json.trim())) json = { url: json.trim() };
  ...
  function loadOnlineSubtitles(url, origin) {
    ...
    onlineGet(absoluteOnlineUrl(origin, url), function (subs) {
      try { if (Lampa.Player.subtitles) Lampa.Player.subtitles(subs); } catch (_) {}
    }, function () {}, true);
  }
  ...
  function beginOnlineResolver(record) {
    ...
    if (!origin) {
      try { origin = String(window.location && window.location.origin || '').replace(/\/+$/, ''); } catch (_) {}
    }
    ...
    ensureOnlineExternalIds(record, origin, function (freshCard) {
      ...
      function useEvents(events, attempts) {
        ...
        if (events && events.life && events.memkey && attempts < 15) {
          setTimeout(function () {
            ...
            onlineGet(lifeUrl, function (next) { useEvents(next, attempts + 1); }, function (err) {
      ...
      onlineGet(eventsUrl, function (events) { useEvents(events, 0); }, function (err) {
  ...
  function resumeTorrent(record) {
    ...
    pullTorrServerViewed(record, function (seconds) { launch(seconds || 0); });
    return true;
  }

  function latestRecord() {
    var db = loadDb();
    var rows = Object.keys(db.items).map(function (key) { return db.items[key]; })
      .filter(function (record) { return !record.profile_id || String(record.profile_id) === profileId(); });
    rows.sort(function (a, b) { return Number(b.updated_at || 0) - Number(a.updated_at || 0); });
    return rows[0] || null;
  }

  function findRecord(recordOrKey) {
    if (recordOrKey && typeof recordOrKey === 'object' && recordOrKey.key) return recordOrKey;
    var db = loadDb();
  ...
  function makeResumeButton(record) {
    var btn = $('<div class="full-start__button selector lampac-resume--button" data-subtitle="Lampac Resume">' +
      '<svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true">' +
      '<path fill="currentColor" d="M8 5v14l11-7z"></path></svg><span></span></div>');
    btn.find('span').text(Core.resumeButtonLabel(record));
    btn.on('hover:enter', function () {
      log('button clicked', { key: record.key, source: record.source && record.source.kind });
      resume(record.key);
    });
    return btn;
  }
  ...
  function renderResumeButton(event) {
    ...
    if (card && typeof card === 'object' && (card.id != null || card.tmdb_id || card.imdb_id || card.kinopoisk_id || card.title || card.name)) {
      lastKnownCard = card;
    }

    if (event.type !== 'complite') {
      log('full event', event.type || 'unknown', { card_id: card && card.id });
      return;
    }
    ...
    var root = null;
    try {
      root = event.object && event.object.activity && event.object.activity.render
    ...
    if (!root || !root.find) {
      log('button skip: full-card root unavailable', { card_id: card.id });
      return;
    }

    lastFullContext = { event: event, card: card, root: root };
    ...
    if (!record) {
      log('button skip: no resumable record', { card_id: card.id, records: listRecords().length });
      return;
    }
    ...
    if (!anchor || !anchor.length) {
      log('button skip: anchor not found', { card_id: card.id });
      return;
    }
  ...
  function refreshCurrentCardButton(reason) {
    ...
    log('button refresh', reason || 'manual', { card_id: lastFullContext.card && lastFullContext.card.id });
    try {
  ...
  function refreshFromActiveActivity(reason) {
    ...
    var candidates = [
      active.activity,
      active.object && active.object.activity,
      active.component,
    ...
    if (!activity) {
      log('active full probe: renderer unavailable', { reason: reason || '', card_id: card.id });
      return false;
    }

    log('active full probe: rendering button', { reason: reason || '', card_id: card.id });
    renderResumeButton({
      type: 'complite',
      data: { movie: card },
  ...
  adapters['lampac-online'] = { resume: resumeOnline };
  adapters['torrserver'] = { resume: resumeTorrent };
  ...
  function start() {
    ...
    Lampa.LampacResume = {
      ...
      pullTorrServer: function (recordOrKey, callback) {
        var record = findRecord(recordOrKey);
        return pullTorrServerViewed(record, callback);
      },
      pushTorrServer: function (recordOrKey) {
        var record = findRecord(recordOrKey);
