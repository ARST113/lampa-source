(function () {
  ...
  function pickResume(full) {
    ...
    for (i = 0; i < list.length; i++) {
      ...
      if (!started || resumeUpdated(list[i]) > resumeUpdated(started) ||
          (resumeUpdated(list[i]) === resumeUpdated(started) && list[i].index > started.index)) {
        started = list[i];
      }
    }
  ...
  function buildHero() {
    ...
    var percent = heroProgress(target, season);
    if (percent > 0) {
  ...
  function stamp() {
    return [
      ...
      ui_page,
      ui_all_sources ? 1 : 0,
      items.length ? stableItemTitle(items[0]) : '',
